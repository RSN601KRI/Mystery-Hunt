...
export const useMysteryHunt = () => {
  ...
  const [userContent, setUserContent] = useState({
    ...
    answer: '',
    difficulty: 1,
    setClue: (clue) => setUserContent(prev => ({ ...prev, clue })),
    setAnswer: (answer) => setUserContent(prev => ({ ...prev, answer })),
    setDifficulty: (difficulty) => setUserContent(prev => ({ ...prev, difficulty }))
  });
  ...
  const useHint = useCallback((hintNumber) => {
  ...
  const voteOnContent = useCallback((index, voteType) => {
    setCurrentChallenge(prev => ({
      ...prev,
